...
// Lấy thông tin IP, ISP, Server từ Cloudflare
async function fetchServerInfo() {
    try {
        const res = await fetch('https://speed.cloudflare.com/meta');
        const data = await res.json();

        // clientIp có thể là string trực tiếp
        document.getElementById('infoIP').textContent = typeof data.clientIp === 'string' ? data.clientIp : JSON.stringify(data.clientIp);
        document.getElementById('infoISP').textContent = data.asOrganization || '-';
        document.getElementById('infoCity').textContent = [data.city, data.country].filter(Boolean).join(', ') || '-';
        // colo là mã sân bay 3 chữ (vd: SGN, HAN)
        document.getElementById('infoServer').textContent = data.colo || '-';
    } catch (e) {
...
async function startTest() {
    ...
    // Ẩn results và server info khi bắt đầu test mới
    ...
    try {
        ...
        speedtest.onFinish = (results) => {
            ...
            // Hiện server info bên dưới results sau khi test xong
...
loadHistory();
fetchServerInfo(); // Fetch ngay khi load trang nhưng chỉ hiện sau khi test xong
